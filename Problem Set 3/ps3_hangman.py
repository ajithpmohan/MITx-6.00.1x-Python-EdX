...
def hangman(secretWord):
    ...
    * After each round, you should also display to the user the 
    ...
    while (count>0) and ('_' in result):
      avail=getAvailableLetters(lettersGuessed)
      print('-------------')
      print('You have '+ str(count) +' guesses left.')
      print('Available letters: ' + avail)
      guess=raw_input('Please guess a letter: ').lower()
      if guess in secretWord and guess in avail:
      	lettersGuessed.append(guess)
      	result=getGuessedWord(secretWord, lettersGuessed)
      	print 'Good guess: '+str(result)
      elif guess not in secretWord and guess not in lettersGuessed:
        lettersGuessed.append(guess)
        count-=1
        result=getGuessedWord(secretWord, lettersGuessed)
        print 'Oops! That letter is not in my word: '+str(result)
      else:
      	result=getGuessedWord(secretWord, lettersGuessed)
      	print 'Oops! You\'ve already guessed that letter: '+str(result)
    print('-------------')
    if '_' not in result:
